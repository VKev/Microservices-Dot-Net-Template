...
# Route `/s3/*` on the primary hostname directly to the S3 bucket and override the static hostname to talk to the bucket origin.
resource "cloudflare_ruleset" "origin_overrides" {
  count   = var.use_cloudflare && var.static_assets_bucket_domain_name != "" ? 1 : 0
  zone_id = var.cloudflare_zone_id
  name    = "S3 origin overrides"
...
# Explicitly enable edge caching for both `/s3/*` on the primary host and the static assets hostname.
resource "cloudflare_ruleset" "cache_settings" {
  count   = var.use_cloudflare && var.static_assets_bucket_domain_name != "" ? 1 : 0
  zone_id = var.cloudflare_zone_id
  name    = "S3 cache settings"
