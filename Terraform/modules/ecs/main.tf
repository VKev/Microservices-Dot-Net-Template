...
resource "null_resource" "wait_for_dependencies" {
  ...
  triggers = {
    ...
    upstream_dependency_waiter_hash = sha1(join(",", var.upstream_dependency_waiter_ids))
  }

  provisioner "local-exec" {
    command = "python3 -u ${path.module}/../../scripts/wait_for_services.py --cluster ${var.ecs_cluster_name} --services ${join(",", [for s in each.value : "${var.project_name}-${s}"])} --region ${var.aws_region} --timeout 20"
  }
}
